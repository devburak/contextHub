...
import { $createParagraphNode, $getNearestNodeFromDOMNode, $getNodeByKey, $getRoot, COMMAND_PRIORITY_HIGH, COMMAND_PRIORITY_LOW, DRAGOVER_COMMAND, DROP_COMMAND } from 'lexical';
...
function isOnMenu(element) {
    var _a;
    return !!((_a = element.closest) === null || _a === void 0 ? void 0 : _a.call(element, `.${DRAGGABLE_BLOCK_MENU_CLASSNAME}`));
}
...
function useDraggableBlockMenu(editor, anchorElem, menuRef, targetLineRef, isEditable, menuComponent, targetLineComponent, isOnMenu, onElementChanged) {
    ...
    return createPortal(React.createElement(React.Fragment, null,
        React.createElement("div", { draggable: true, onDragStart: onDragStart, onDragEnd: onDragEnd }, isEditable && menuComponent),
        targetLineComponent), anchorElem);
}
export function DraggableBlockPlugin_EXPERIMENTAL({ anchorElem = document.body, menuRef, targetLineRef, menuComponent, targetLineComponent, isOnMenu, onElementChanged, }) {
...
export default function DraggableBlockPlugin({ anchorElem, } = {}) {
    ...
    const resolvedAnchorElem = anchorElem !== null && anchorElem !== void 0 ? anchorElem : (typeof document !== 'undefined' ? document.body : null);
    const insertBlock = useCallback((event) => {
        if (!draggableElement) {
            return;
        }
        editor.update(() => {
    ...
    return (React.createElement(DraggableBlockPlugin_EXPERIMENTAL, { anchorElem: resolvedAnchorElem, menuRef: menuRef, targetLineRef: targetLineRef, menuComponent: React.createElement("div", { ref: menuRef, className: `icon ${DRAGGABLE_BLOCK_MENU_CLASSNAME}` },
            React.createElement("button", { type: "button", title: "Yeni blok ekle", className: "icon icon-plus", onMouseDown: (event) => event.preventDefault(), onClick: insertBlock }),
            React.createElement("div", { className: "icon" })), targetLineComponent: React.createElement("div", { ref: targetLineRef, className: "draggable-block-target-line" }), isOnMenu: isOnMenu, onElementChanged: setDraggableElement }));
}
