...
export default function Billing() {
  ...
  const overview = useQuery({
    ...
    refetchInterval: (data) => data?.tenant?.status === 'pending_payment' ? 5000 : false,
  })
                  ...
                  <div className="mt-4 space-y-2">
                    <details id="billing-terms" className="rounded-xl border border-[var(--billing-line)] bg-white p-4 open:shadow-sm">
                      ...
                      <ul className="mt-4 space-y-2 border-t border-[var(--billing-line)] pt-4 text-xs leading-5 text-[var(--billing-muted)]">
                        {['termsProvider', 'termsCompanyDetails', 'termsScope', 'termsBilling', 'termsQuota', 'termsInfrastructure', 'termsCancellation', 'termsSecurity'].map((key) => <li key={key} className="flex gap-2"><span aria-hidden="true">•</span><span>{t(`billing.legal.${key}`)}</span></li>)}
                      </ul>
                    </details>
